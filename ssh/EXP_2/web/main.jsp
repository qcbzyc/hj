    ...
    <script type="text/javascript" >
       ...
       $(function(){
                   ...
                   success:function(res){
                       ...
                       for (var i=0;i<result.length;i++){
                           str+='<tr><td>'+result[i].empID+'</td><td>'+result[i].empName+'</td><td>'+result[i].job+'</td><td>'+result[i].salary+'</td><td>'+result[i].dept+'</td>'+
                               ...
                               '<button class="btn btn-danger btn-xs del" data-toggle="modal" data-target="#myModal">删除</button></td></tr>'
                       }
                       //刷新前先清除第一行以外的数据
           ...
           $(document).on("click",".del",function(){
               ...
               empID = $(this).parent().siblings().eq(0).text()
               //动态填充弹窗内容
               document.getElementById("myModalLabel").innerHTML="删除"
               document.getElementById("modal-body").innerHTML="是否确认删除员工"+empName+"?"
               $(".btn-primary0").click(function(){
                   var employee={
                       "empID":empID
                   }
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" class="btn btn-primary0">确认</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
